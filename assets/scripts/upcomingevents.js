...
imprimirTarjetas(filtrarEventosFuturos(data));
...
function categoriasSeleccionadas(checkedActivo){
    if(checkedActivo){          
        return filtrarEventosFuturos(data).filter(evento => checkedActivo.includes(evento.category));
    }           
} 
obtenerCategorias()
...
function filtrarPorBuscador(valor){
    ...
    let tarjetasFiltradas=filtrarEventosFuturos(data).filter(evento=>evento.description.toLocaleLowerCase().includes(valorBusqueda)||evento.name.toLocaleLowerCase().includes(valorBusqueda))
